...
LOG_FILE = Path(__file__).parent / "pipeline_new_species.log"
STATE_FILE = Path(__file__).parent / "pipeline_new_species_state.json"
...
TAXONOMY_CSV = PROJECT_ROOT / "final_taxonomy_occ.csv"
SEARCH_PROGRESS = PROJECT_ROOT / "search_progress.json"
DOWNLOAD_PROGRESS = PROJECT_ROOT / "download_progress.json"
...
def run_index_phase(state: dict[str, Any]) -> None:
    ...
    for pdf_dir, label in [(PDFS_DIR, "pdfs/"), (OUTPUTS_PDFS_DIR, "outputs/pdfs/")]:
        ...
        cmd = [
            sys.executable, str(PROJECT_ROOT / "indexar.py"),
            "--pdf-dir", str(pdf_dir),
