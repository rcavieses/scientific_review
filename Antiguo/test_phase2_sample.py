#!/usr/bin/env python3
"""
Script de prueba del pipeline Fase 2 con 2% de especies (231 especies iniciales).

Uso:
    python test_phase2_sample.py
    python test_phase2_sample.py --verbose
"""

import csv
import sys
import time
import argparse
from pathlib import Path
import logging

# Configurar logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s"
)
logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).resolve().parent
sys.path.insert(0, str(PROJECT_ROOT))

from scripts.phase_2_search.search_articles import search_articles_batch

def get_sample_species(csv_path: Path, sample_percent: float = 0.02) -> list[str]:
    """Extrae muestra de especies del CSV."""
    species_list = []
    with open(csv_path, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for i, row in enumerate(reader):
            species = (row.get('Especie') or row.get('species') or "").strip()
            if species:
                species_list.append(species)
    
    sample_size = max(1, int(len(species_list) * sample_percent))
    sample = species_list[:sample_size]
    
    logger.info(f"Total especies en archivo: {len(species_list)}")
    logger.info(f"Muestra ({sample_percent*100:.0f}%): {len(sample)} especies")
    
    return sample

def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--verbose', action='store_true', help='Verbose output')
    parser.add_argument('--sample-percent', type=float, default=0.02, help='Sample percentage')
    args = parser.parse_args()
    
    # Paths
    csv_path = PROJECT_ROOT / "data/input/final_taxonomy_occ.csv"
    output_dir = PROJECT_ROOT / "outputs/search_results_test"
    progress_file = PROJECT_ROOT / "outputs/state/test_search_progress.json"
    
    if not csv_path.exists():
        logger.error(f"Archivo no encontrado: {csv_path}")
        sys.exit(1)
    
    # Obtener muestra
    species_list = get_sample_species(csv_path, args.sample_percent)
    
    # Crear directorios
    output_dir.mkdir(parents=True, exist_ok=True)
    progress_file.parent.mkdir(parents=True, exist_ok=True)
    
    logger.info("")
    logger.info("=" * 80)
    logger.info(f"INICIANDO BÚSQUEDA - {len(species_list)} especies")
    logger.info("=" * 80)
    logger.info(f"Salida: {output_dir}")
    logger.info(f"Progreso: {progress_file}")
    logger.info("")
    
    # Ejecutar búsqueda
    inicio = time.time()
    results = search_articles_batch(
        species_list=species_list,
        output_dir=output_dir,
        progress_file=progress_file
    )
    tiempo_total = time.time() - inicio
    
    # Reportar resultados
    logger.info("")
    logger.info("=" * 80)
    logger.info("RESUMEN DE RESULTADOS")
    logger.info("=" * 80)
    logger.info(f"Total procesadas: {results['processed']}")
    logger.info(f"Con artículos: {results['found']}")
    logger.info(f"Éxito: {results['found']/max(1, results['processed'])*100:.1f}%")
    logger.info(f"Tiempo total: {tiempo_total/60:.1f} minutos")
    logger.info(f"Tiempo promedio por especie: {tiempo_total/results['processed']:.1f}s")
    logger.info(f"Estimado para 11,558 especies: {tiempo_total/results['processed']*11558/3600:.1f} horas")
    logger.info("=" * 80)
    logger.info("")
    
    # Listar algunos archivos generados
    csv_files = list(output_dir.glob("*.csv"))
    logger.info(f"Archivos CSV generados: {len(csv_files)}")
    if csv_files:
        logger.info("Primeros 5 archivos:")
        for csv_file in csv_files[:5]:
            size = csv_file.stat().st_size / 1024
            logger.info(f"  - {csv_file.name} ({size:.1f} KB)")

if __name__ == "__main__":
    main()
