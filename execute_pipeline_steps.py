...
def load_progress(step: int) -> dict[str, Any]:
    ...
    progress_file = PROJECT_ROOT / progress_files.get(step, "")
...
def save_execution_state(state: dict[str, Any]) -> None:
    ...
    state_file = PROJECT_ROOT / "pipeline_execution_state.json"
...
def execute_step_3() -> dict[str, Any]:
    ...
    try:
        ...
        # Ejecutar búsqueda
        progress_file = PROJECT_ROOT / "search_progress.json"
        results = search_articles_batch(species_list, PROJECT_ROOT / "search_results", progress_file)
...
def execute_step_4() -> dict[str, Any]:
    ...
    try:
        ...
        progress_file = PROJECT_ROOT / "download_progress.json"
        results = download_all_articles(
            PROJECT_ROOT / "search_results",
            PROJECT_ROOT / "pdfs",
            progress_file,
        )
